 ...
 *
 * @see <a href="https://en.wikipedia.org/wiki/Tseytin_transformation">Wikipedia: Tseytin
 *     Transformation</a>
...
//    }
  ...
  public void bddToDimacsTseytinTransformation(BDDFeatureExpr expr, OutputStream out) {
    ...
    Set<SingleFeatureExpr> features =
        JavaConversions.setAsJavaSet(expr.collectDistinctFeatureObjects());
    printToDimacs(features, variables, expressions, out);
  }

  private Collection<SingleFeatureExpr> createExpressions(
      BDDFeatureExpr expr, Collection<FeatureExpr> expressions) {
    ...
    while (!stack.isEmpty()) {
      ...
      SingleFeatureExpr feature =
          FeatureExprFactory.createDefinedExternal(FExprBuilder.lookupFeatureName(bdd.var()));
  ...
  private FeatureExpr createExpr(
      FeatureExpr feature, BDD next, Map<BDD, SingleFeatureExpr> nodeVariables) {
  ...
  private SingleFeatureExpr getOrCreateVariable(
      BDD bdd, Map<BDD, SingleFeatureExpr> nodeVariables) {
    return nodeVariables.computeIfAbsent(
        bdd, k -> FeatureExprFactory.createDefinedExternal("nodevar" + bdd.hashCode()));
  }

  private void printToDimacs(
      Collection<SingleFeatureExpr> features,
      Collection<SingleFeatureExpr> nodeVariables,
      Collection<FeatureExpr> expressions,
      OutputStream stream) {
  ...
  private void printClauses(
      List<Collection<Integer>> clauses, Map<String, Integer> featureMap, PrintWriter out) {
...
//    throw new RuntimeException(Conditional.getCTXString(ctx));
  ...
  private int printVariables(
      Collection<SingleFeatureExpr> nodes,
